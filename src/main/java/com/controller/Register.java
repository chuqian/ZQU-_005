...
import com.cache.RedisCacheServiceImpl;
...
public class Register {
	...
	public ModelAndView registerSaller(Seller seller) {
		...
		sellerService.saveSeller(sBaseDao, seller);
	...
	public ModelAndView registerCustomer(Customer customer) {
		...
		customerService.saveCustomer(cBaseDao, customer);
	...
	public ModelAndView registerAccount(User user) {
		...
		if(userService.validateEmail(uBaseDao, user.getId()) != NO_ROLE) //此账号没有注册过买家和卖家
			userService.saveUser(uBaseDao, user);
		else
			userService.updateUser(uBaseDao, user);	//此账号已经注册过卖家或买家其中一个角色账号
	...
	public void getCode(HttpServletRequest request, HttpServletResponse response) {
		...
		redisCacheServiceImpl.set(code, code);
		redisCacheServiceImpl.expire(code, 60);
		redisCacheServiceImpl.set(email, email);
		redisCacheServiceImpl.expire(email, 60);
	...
	public void validateCode(HttpServletRequest request, HttpServletResponse response) {
		...
		String afterCode = redisCacheServiceImpl.get(preCode);
		String afterEmail = redisCacheServiceImpl.get(preEmail);
	...
	public void validateEmail(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			if(userService.validateEmail(role, userService.validateEmail(uBaseDao, email)))
				response.getWriter().write("1");
			else
				response.getWriter().write("0");
		} catch (Exception e) {
